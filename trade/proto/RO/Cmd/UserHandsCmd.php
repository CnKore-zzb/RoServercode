<?php
// DO NOT EDIT! Generated by Protobuf-PHP protoc plugin @package_version@
// Source: SceneMap.proto

namespace RO\Cmd {

  class UserHandsCmd extends \DrSlump\Protobuf\Message {

    /**  @var int - \RO\Cmd\Command */
    public $cmd = \RO\Cmd\Command::SCENE_USER_MAP_PROTOCMD;
    
    /**  @var int - \RO\Cmd\MapParam */
    public $param = \RO\Cmd\MapParam::MAPPARAM_USER_HANDS;
    
    /**  @var int */
    public $player1 = 0;
    
    /**  @var int */
    public $player2 = 0;
    
    /**  @var int */
    public $isadd = 1;
    

    /** @var \Closure[] */
    protected static $__extensions = array();

    public static function descriptor()
    {
      $descriptor = new \DrSlump\Protobuf\Descriptor(__CLASS__, 'Cmd.UserHandsCmd');

      // OPTIONAL ENUM cmd = 1
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 1;
      $f->name      = "cmd";
      $f->type      = \DrSlump\Protobuf::TYPE_ENUM;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->reference = '\RO\Cmd\Command';
      $f->default   = \RO\Cmd\Command::SCENE_USER_MAP_PROTOCMD;
      $descriptor->addField($f);

      // OPTIONAL ENUM param = 2
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 2;
      $f->name      = "param";
      $f->type      = \DrSlump\Protobuf::TYPE_ENUM;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->reference = '\RO\Cmd\MapParam';
      $f->default   = \RO\Cmd\MapParam::MAPPARAM_USER_HANDS;
      $descriptor->addField($f);

      // OPTIONAL UINT64 player1 = 3
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 3;
      $f->name      = "player1";
      $f->type      = \DrSlump\Protobuf::TYPE_UINT64;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->default   = 0;
      $descriptor->addField($f);

      // OPTIONAL UINT64 player2 = 4
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 4;
      $f->name      = "player2";
      $f->type      = \DrSlump\Protobuf::TYPE_UINT64;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->default   = 0;
      $descriptor->addField($f);

      // OPTIONAL UINT32 isadd = 5
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 5;
      $f->name      = "isadd";
      $f->type      = \DrSlump\Protobuf::TYPE_UINT32;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->default   = 1;
      $descriptor->addField($f);

      foreach (self::$__extensions as $cb) {
        $descriptor->addField($cb(), true);
      }

      return $descriptor;
    }

    /**
     * Check if <cmd> has a value
     *
     * @return boolean
     */
    public function hasCmd(){
      return $this->_has(1);
    }
    
    /**
     * Clear <cmd> value
     *
     * @return \RO\Cmd\UserHandsCmd
     */
    public function clearCmd(){
      return $this->_clear(1);
    }
    
    /**
     * Get <cmd> value
     *
     * @return int - \RO\Cmd\Command
     */
    public function getCmd(){
      return $this->_get(1);
    }
    
    /**
     * Set <cmd> value
     *
     * @param int - \RO\Cmd\Command $value
     * @return \RO\Cmd\UserHandsCmd
     */
    public function setCmd( $value){
      return $this->_set(1, $value);
    }
    
    /**
     * Check if <param> has a value
     *
     * @return boolean
     */
    public function hasParam(){
      return $this->_has(2);
    }
    
    /**
     * Clear <param> value
     *
     * @return \RO\Cmd\UserHandsCmd
     */
    public function clearParam(){
      return $this->_clear(2);
    }
    
    /**
     * Get <param> value
     *
     * @return int - \RO\Cmd\MapParam
     */
    public function getParam(){
      return $this->_get(2);
    }
    
    /**
     * Set <param> value
     *
     * @param int - \RO\Cmd\MapParam $value
     * @return \RO\Cmd\UserHandsCmd
     */
    public function setParam( $value){
      return $this->_set(2, $value);
    }
    
    /**
     * Check if <player1> has a value
     *
     * @return boolean
     */
    public function hasPlayer1(){
      return $this->_has(3);
    }
    
    /**
     * Clear <player1> value
     *
     * @return \RO\Cmd\UserHandsCmd
     */
    public function clearPlayer1(){
      return $this->_clear(3);
    }
    
    /**
     * Get <player1> value
     *
     * @return int
     */
    public function getPlayer1(){
      return $this->_get(3);
    }
    
    /**
     * Set <player1> value
     *
     * @param int $value
     * @return \RO\Cmd\UserHandsCmd
     */
    public function setPlayer1( $value){
      return $this->_set(3, $value);
    }
    
    /**
     * Check if <player2> has a value
     *
     * @return boolean
     */
    public function hasPlayer2(){
      return $this->_has(4);
    }
    
    /**
     * Clear <player2> value
     *
     * @return \RO\Cmd\UserHandsCmd
     */
    public function clearPlayer2(){
      return $this->_clear(4);
    }
    
    /**
     * Get <player2> value
     *
     * @return int
     */
    public function getPlayer2(){
      return $this->_get(4);
    }
    
    /**
     * Set <player2> value
     *
     * @param int $value
     * @return \RO\Cmd\UserHandsCmd
     */
    public function setPlayer2( $value){
      return $this->_set(4, $value);
    }
    
    /**
     * Check if <isadd> has a value
     *
     * @return boolean
     */
    public function hasIsadd(){
      return $this->_has(5);
    }
    
    /**
     * Clear <isadd> value
     *
     * @return \RO\Cmd\UserHandsCmd
     */
    public function clearIsadd(){
      return $this->_clear(5);
    }
    
    /**
     * Get <isadd> value
     *
     * @return int
     */
    public function getIsadd(){
      return $this->_get(5);
    }
    
    /**
     * Set <isadd> value
     *
     * @param int $value
     * @return \RO\Cmd\UserHandsCmd
     */
    public function setIsadd( $value){
      return $this->_set(5, $value);
    }
  }
}


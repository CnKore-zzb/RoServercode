<?php
// DO NOT EDIT! Generated by Protobuf-PHP protoc plugin @package_version@
// Source: SessionTeam.proto

namespace RO\Cmd {

  class UpdateHelpWantedTeamCmd extends \DrSlump\Protobuf\Message {

    /**  @var int - \RO\Cmd\Command */
    public $cmd = \RO\Cmd\Command::SESSION_USER_TEAM_PROTOCMD;
    
    /**  @var int - \RO\Cmd\TeamParam */
    public $param = \RO\Cmd\TeamParam::TEAMPARAM_UPDATEHELPWANTED;
    
    /**  @var int[]  */
    public $addlist = array();
    
    /**  @var int[]  */
    public $dellist = array();
    

    /** @var \Closure[] */
    protected static $__extensions = array();

    public static function descriptor()
    {
      $descriptor = new \DrSlump\Protobuf\Descriptor(__CLASS__, 'Cmd.UpdateHelpWantedTeamCmd');

      // OPTIONAL ENUM cmd = 1
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 1;
      $f->name      = "cmd";
      $f->type      = \DrSlump\Protobuf::TYPE_ENUM;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->reference = '\RO\Cmd\Command';
      $f->default   = \RO\Cmd\Command::SESSION_USER_TEAM_PROTOCMD;
      $descriptor->addField($f);

      // OPTIONAL ENUM param = 2
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 2;
      $f->name      = "param";
      $f->type      = \DrSlump\Protobuf::TYPE_ENUM;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->reference = '\RO\Cmd\TeamParam';
      $f->default   = \RO\Cmd\TeamParam::TEAMPARAM_UPDATEHELPWANTED;
      $descriptor->addField($f);

      // REPEATED UINT32 addlist = 3
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 3;
      $f->name      = "addlist";
      $f->type      = \DrSlump\Protobuf::TYPE_UINT32;
      $f->rule      = \DrSlump\Protobuf::RULE_REPEATED;
      $descriptor->addField($f);

      // REPEATED UINT32 dellist = 4
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 4;
      $f->name      = "dellist";
      $f->type      = \DrSlump\Protobuf::TYPE_UINT32;
      $f->rule      = \DrSlump\Protobuf::RULE_REPEATED;
      $descriptor->addField($f);

      foreach (self::$__extensions as $cb) {
        $descriptor->addField($cb(), true);
      }

      return $descriptor;
    }

    /**
     * Check if <cmd> has a value
     *
     * @return boolean
     */
    public function hasCmd(){
      return $this->_has(1);
    }
    
    /**
     * Clear <cmd> value
     *
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function clearCmd(){
      return $this->_clear(1);
    }
    
    /**
     * Get <cmd> value
     *
     * @return int - \RO\Cmd\Command
     */
    public function getCmd(){
      return $this->_get(1);
    }
    
    /**
     * Set <cmd> value
     *
     * @param int - \RO\Cmd\Command $value
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function setCmd( $value){
      return $this->_set(1, $value);
    }
    
    /**
     * Check if <param> has a value
     *
     * @return boolean
     */
    public function hasParam(){
      return $this->_has(2);
    }
    
    /**
     * Clear <param> value
     *
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function clearParam(){
      return $this->_clear(2);
    }
    
    /**
     * Get <param> value
     *
     * @return int - \RO\Cmd\TeamParam
     */
    public function getParam(){
      return $this->_get(2);
    }
    
    /**
     * Set <param> value
     *
     * @param int - \RO\Cmd\TeamParam $value
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function setParam( $value){
      return $this->_set(2, $value);
    }
    
    /**
     * Check if <addlist> has a value
     *
     * @return boolean
     */
    public function hasAddlist(){
      return $this->_has(3);
    }
    
    /**
     * Clear <addlist> value
     *
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function clearAddlist(){
      return $this->_clear(3);
    }
    
    /**
     * Get <addlist> value
     *
     * @param int $idx
     * @return int
     */
    public function getAddlist($idx = NULL){
      return $this->_get(3, $idx);
    }
    
    /**
     * Set <addlist> value
     *
     * @param int $value
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function setAddlist( $value, $idx = NULL){
      return $this->_set(3, $value, $idx);
    }
    
    /**
     * Get all elements of <addlist>
     *
     * @return int[]
     */
    public function getAddlistList(){
     return $this->_get(3);
    }
    
    /**
     * Add a new element to <addlist>
     *
     * @param int $value
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function addAddlist( $value){
     return $this->_add(3, $value);
    }
    
    /**
     * Check if <dellist> has a value
     *
     * @return boolean
     */
    public function hasDellist(){
      return $this->_has(4);
    }
    
    /**
     * Clear <dellist> value
     *
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function clearDellist(){
      return $this->_clear(4);
    }
    
    /**
     * Get <dellist> value
     *
     * @param int $idx
     * @return int
     */
    public function getDellist($idx = NULL){
      return $this->_get(4, $idx);
    }
    
    /**
     * Set <dellist> value
     *
     * @param int $value
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function setDellist( $value, $idx = NULL){
      return $this->_set(4, $value, $idx);
    }
    
    /**
     * Get all elements of <dellist>
     *
     * @return int[]
     */
    public function getDellistList(){
     return $this->_get(4);
    }
    
    /**
     * Add a new element to <dellist>
     *
     * @param int $value
     * @return \RO\Cmd\UpdateHelpWantedTeamCmd
     */
    public function addDellist( $value){
     return $this->_add(4, $value);
    }
  }
}


<?php
// DO NOT EDIT! Generated by Protobuf-PHP protoc plugin @package_version@
// Source: SceneUser2.proto

namespace RO\Cmd {

  class ItemMusicNtfUserCmd extends \DrSlump\Protobuf\Message {

    /**  @var int - \RO\Cmd\Command */
    public $cmd = \RO\Cmd\Command::SCENE_USER2_PROTOCMD;
    
    /**  @var int - \RO\Cmd\User2Param */
    public $param = \RO\Cmd\User2Param::USER2PARAM_ITEM_MUSIC_NTF;
    
    /**  @var boolean */
    public $add = null;
    
    /**  @var string */
    public $uri = null;
    
    /**  @var int */
    public $starttime = null;
    

    /** @var \Closure[] */
    protected static $__extensions = array();

    public static function descriptor()
    {
      $descriptor = new \DrSlump\Protobuf\Descriptor(__CLASS__, 'Cmd.ItemMusicNtfUserCmd');

      // OPTIONAL ENUM cmd = 1
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 1;
      $f->name      = "cmd";
      $f->type      = \DrSlump\Protobuf::TYPE_ENUM;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->reference = '\RO\Cmd\Command';
      $f->default   = \RO\Cmd\Command::SCENE_USER2_PROTOCMD;
      $descriptor->addField($f);

      // OPTIONAL ENUM param = 2
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 2;
      $f->name      = "param";
      $f->type      = \DrSlump\Protobuf::TYPE_ENUM;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $f->reference = '\RO\Cmd\User2Param';
      $f->default   = \RO\Cmd\User2Param::USER2PARAM_ITEM_MUSIC_NTF;
      $descriptor->addField($f);

      // OPTIONAL BOOL add = 3
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 3;
      $f->name      = "add";
      $f->type      = \DrSlump\Protobuf::TYPE_BOOL;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $descriptor->addField($f);

      // OPTIONAL STRING uri = 4
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 4;
      $f->name      = "uri";
      $f->type      = \DrSlump\Protobuf::TYPE_STRING;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $descriptor->addField($f);

      // OPTIONAL UINT32 starttime = 5
      $f = new \DrSlump\Protobuf\Field();
      $f->number    = 5;
      $f->name      = "starttime";
      $f->type      = \DrSlump\Protobuf::TYPE_UINT32;
      $f->rule      = \DrSlump\Protobuf::RULE_OPTIONAL;
      $descriptor->addField($f);

      foreach (self::$__extensions as $cb) {
        $descriptor->addField($cb(), true);
      }

      return $descriptor;
    }

    /**
     * Check if <cmd> has a value
     *
     * @return boolean
     */
    public function hasCmd(){
      return $this->_has(1);
    }
    
    /**
     * Clear <cmd> value
     *
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function clearCmd(){
      return $this->_clear(1);
    }
    
    /**
     * Get <cmd> value
     *
     * @return int - \RO\Cmd\Command
     */
    public function getCmd(){
      return $this->_get(1);
    }
    
    /**
     * Set <cmd> value
     *
     * @param int - \RO\Cmd\Command $value
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function setCmd( $value){
      return $this->_set(1, $value);
    }
    
    /**
     * Check if <param> has a value
     *
     * @return boolean
     */
    public function hasParam(){
      return $this->_has(2);
    }
    
    /**
     * Clear <param> value
     *
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function clearParam(){
      return $this->_clear(2);
    }
    
    /**
     * Get <param> value
     *
     * @return int - \RO\Cmd\User2Param
     */
    public function getParam(){
      return $this->_get(2);
    }
    
    /**
     * Set <param> value
     *
     * @param int - \RO\Cmd\User2Param $value
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function setParam( $value){
      return $this->_set(2, $value);
    }
    
    /**
     * Check if <add> has a value
     *
     * @return boolean
     */
    public function hasAdd(){
      return $this->_has(3);
    }
    
    /**
     * Clear <add> value
     *
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function clearAdd(){
      return $this->_clear(3);
    }
    
    /**
     * Get <add> value
     *
     * @return boolean
     */
    public function getAdd(){
      return $this->_get(3);
    }
    
    /**
     * Set <add> value
     *
     * @param boolean $value
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function setAdd( $value){
      return $this->_set(3, $value);
    }
    
    /**
     * Check if <uri> has a value
     *
     * @return boolean
     */
    public function hasUri(){
      return $this->_has(4);
    }
    
    /**
     * Clear <uri> value
     *
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function clearUri(){
      return $this->_clear(4);
    }
    
    /**
     * Get <uri> value
     *
     * @return string
     */
    public function getUri(){
      return $this->_get(4);
    }
    
    /**
     * Set <uri> value
     *
     * @param string $value
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function setUri( $value){
      return $this->_set(4, $value);
    }
    
    /**
     * Check if <starttime> has a value
     *
     * @return boolean
     */
    public function hasStarttime(){
      return $this->_has(5);
    }
    
    /**
     * Clear <starttime> value
     *
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function clearStarttime(){
      return $this->_clear(5);
    }
    
    /**
     * Get <starttime> value
     *
     * @return int
     */
    public function getStarttime(){
      return $this->_get(5);
    }
    
    /**
     * Set <starttime> value
     *
     * @param int $value
     * @return \RO\Cmd\ItemMusicNtfUserCmd
     */
    public function setStarttime( $value){
      return $this->_set(5, $value);
    }
  }
}


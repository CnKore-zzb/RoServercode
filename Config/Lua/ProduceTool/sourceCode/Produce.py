...
class ExamplePanel(wx.Panel):
    def __init__(self, parent):
        ...
        btlen = 100
        locale = wx.Locale(wx.LANGUAGE_CHINESE)
    ...
    def OnClickProduce(self, event):
        ...
        if fname == '':
            self.logger.AppendText(u'FileName 不可为空!\n')
            return
        ...
        self.logger.AppendText(u"设置时间信息: " + timestr + " -> " + timestr2 + "\n")
        self.logger.AppendText(u"设置文件名为: " + fname + "\n")
        self.logger.AppendText(u"开始生成产出文件......\n")
    ...
    def OnClickSelectF1(self, event):
        ...
        if dialogResult != wx.ID_OK:
            self.logger.AppendText(u"文件1 没有进行选择!\n")
            return

        fname = dlg.GetFilename()
        fnames = fname.split(".")
        if len(fnames) < 2 or fnames[1] != 'csv':
            self.logger.AppendText(u"文件1 选择失败, 请选择csv文件!\n")
            return
        self.filename1 = fnames[0]

        path = dlg.GetPath()
        self.logger.AppendText(u"文件1 选择: " + path + "\n")
    ...
    def OnClickSelectF2(self, event):
        ...
        dialogResult = dlg.ShowModal()
        if dialogResult != wx.ID_OK:
            self.logger.AppendText(u"文件2 没有进行选择!\n")
            return
        fname = dlg.GetFilename()
        fnames = fname.split(".")
        if len(fnames) < 2 or fnames[1] != 'csv':
            self.logger.AppendText(u"文件2 选择失败, 请选择csv文件!\n")
            return
        self.filename2 = fnames[0]

        path = dlg.GetPath()
        self.logger.AppendText(u"文件2 选择: " + path + "\n")
    ...
    def OnClickSelectCompFile(self, event):
        ...
        dialogResult = dlg.ShowModal()
        if dialogResult != wx.ID_OK:
            self.logger.AppendText(u"对比文件 没有进行选择!\n")
            return
        fname = dlg.GetFilename()
        fnames = fname.split(".")
        if len(fnames) < 2 or fnames[1] != 'csv':
            self.logger.AppendText(u"Comparae File 选择失败, 请选择csv文件!\n")
            return
        path = dlg.GetPath()
        self.compFileName = path
        self.logger.AppendText(u"Compare File 选择: " + path + "\n")
    ...
    def OnClickCompare(self, event):
        ...
        if f1 == '' or f2 == '':
            self.logger.AppendText(u"没有选择两个有效对比文件, 请重新选择!\n")
            return
    ...
    def OnCheckSvn(self, event):
        if self.svnCheckBox.IsChecked():
            if os.system('svn st') != 0:
                ...
                return
            self.logger.AppendText(u'已选用svn, 生成时, 将根据开始时间拉取svn对应配置, 生成产出文件!\n')
        else:
            self.logger.AppendText(u'已取消使用svn, 生成时, 将根据当前本地配置, 生成产出文件!\n')
    ...
    def checkFileExist(self):
        if os.path.exists('../Table/Table_ProduceLogic.txt') == False:
            self.logger.AppendText(u'Table目录中没有配置Table_ProduceLogic.txt, 请检查!\n')
            return False
        ...
        for k in stable:
            ...
            if os.path.exists('../Table/Table_'+ tname +'.txt') == False:
                self.logger.AppendText(u'ProduceLogic 配置的产出文件: ' + tname + u', 在Table/目录下找不到, ' + u'行信息: ' + str(k) + '\n')
                findok = False
        return findok
...
class ProduceThread(threading.Thread):
    ...
    def run(self):
        ...
        if lay.svnCheckBox.IsChecked():
            cmd1 = 'svn up -r ' + '\"{' + timestr + '}\"' + ' ../'
            child = subprocess.Popen(cmd1, shell=True)
            child.wait()
        #os.system(cmd1)
        ...
        lay.logger.AppendText(u"生成产出文件成功, " + self.fname + ".csv !\n\n")
